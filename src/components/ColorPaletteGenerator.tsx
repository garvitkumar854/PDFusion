...
const ColorPanel = React.memo(({ color, onToggleLock, onCopy }: { color: ColorInfo; onToggleLock: () => void; onCopy: () => void; }) => {
      ...
      <div className="space-y-1 cursor-pointer" onClick={handleCopy}>
       ...
       <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <TooltipProvider>
           ...
           <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-black/10 hover:bg-black/20" onClick={(e) => {e.stopPropagation(); handleCopy();}}>
...
const FloatingActions = React.memo(({ onGenerate, onAdd, onRemove, onLockAll, canAdd, canRemove, palette }: { onGenerate: () => void; onAdd: () => void; onRemove: () => void; onLockAll: () => void; canAdd: boolean, canRemove: boolean, palette: Palette }) => {
        ...
        <div className="flex items-center gap-1.5 p-2 bg-card/80 dark:bg-card/50 backdrop-blur-md border rounded-full shadow-lg">
          ...
          <TooltipProvider>
              <Tooltip>
                  ...
                  <TooltipContent><p>Lock/Unlock All</p></TooltipContent>
              </Tooltip>
          </TooltipProvider>
          <Popover>
...
export default function ColorPaletteGenerator() {
  ...
  const lockAll = useCallback(() => {
    ...
    toast({
        title: allLocked ? "All Unlocked" : "All Locked",
        description: allLocked ? "All colors are now ready to be changed." : "All colors are now locked in place.",
        variant: "info",
    })
  }, [palette, toast]);
  ...
  return (
    <div className="w-full h-full flex flex-col">
        ...
        <div className="hidden md:flex items-center justify-center gap-2 text-sm font-medium pb-4 text-muted-foreground">
